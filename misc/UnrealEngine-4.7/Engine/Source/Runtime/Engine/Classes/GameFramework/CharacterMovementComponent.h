...
protected:
	...
	/**
	* Instantaneous force caused by hitting.
	*/
	UPROPERTY()
	FVector ReceivedImpulse;
	UPROPERTY()
	FVector LastReceivedImpulse;
	UPROPERTY()
	FVector ReceivedImpulseClient;
	
	FSavedMovePtr FreezeMove;

	/**
	* WORKAROUND FOR PACKET LOSS ISSUES:
	* Client-side matters only.
	* Is set to 'true' until the server acknowledges the 'ReceivedImpulse' move.
	*/
	uint32 bReceivedImpulsePendent : 1;

	/**
	* WORKAROUND FOR PACKET LOSS ISSUES:
	* Delayed 'JumpInput' clearance after the server acknowledges the 'jump' move.
	*/
	FTimerHandle ClearJumpTimerHandle;
	FTimerDelegate ClearJumpTimerDelegate;
	UFUNCTION()
	void ClearJump();
...
protected:
	...
	*/
	...
	virtual void ReplicateMoveToServer(float DeltaTime, const FVector& NewAcceleration, const FVector& NewReceivedImpulse);
	...
	virtual void MoveAutonomous(float ClientTimeStamp, float DeltaTime, uint8 CompressedFlags, const FVector& NewAccel, const FVector& NewReceivedImpulse);
...
public:
	...
	virtual void ServerMove(float TimeStamp, FVector_NetQuantize10 InAccel, FVector_NetQuantize10 InReceivedImpulse, FVector_NetQuantize100 ClientLoc, uint8 CompressedMoveFlags, uint8 ClientRoll, uint32 View, UPrimitiveComponent* ClientMovementBase, FName ClientBaseBoneName, uint8 ClientMovementMode);
	virtual void ServerMove_Implementation(float TimeStamp, FVector_NetQuantize10 InAccel, FVector_NetQuantize10 InReceivedImpulse, FVector_NetQuantize100 ClientLoc, uint8 CompressedMoveFlags, uint8 ClientRoll, uint32 View, UPrimitiveComponent* ClientMovementBase, FName ClientBaseBoneName, uint8 ClientMovementMode);
	virtual bool ServerMove_Validate(float TimeStamp, FVector_NetQuantize10 InAccel, FVector_NetQuantize10 InReceivedImpulse, FVector_NetQuantize100 ClientLoc, uint8 CompressedMoveFlags, uint8 ClientRoll, uint32 View, UPrimitiveComponent* ClientMovementBase, FName ClientBaseBoneName, uint8 ClientMovementMode);

	/** Replicated function sent by client to server - contains client movement and view info for two moves. */
	UFUNCTION(unreliable, server, WithValidation)
	virtual void ServerMoveDual(float TimeStamp0, FVector_NetQuantize10 InAccel0, FVector_NetQuantize10 InReceivedImp0, uint8 PendingFlags, uint32 View0, float TimeStamp, FVector_NetQuantize10 InAccel, FVector_NetQuantize10 ReceivedImp, FVector_NetQuantize100 ClientLoc, uint8 NewFlags, uint8 ClientRoll, uint32 View, UPrimitiveComponent* ClientMovementBase, FName ClientBaseBoneName, uint8 ClientMovementMode);
	virtual void ServerMoveDual_Implementation(float TimeStamp0, FVector_NetQuantize10 InAccel0, FVector_NetQuantize10 InReceivedImp0, uint8 PendingFlags, uint32 View0, float TimeStamp, FVector_NetQuantize10 InAccel, FVector_NetQuantize10 ReceivedImp, FVector_NetQuantize100 ClientLoc, uint8 NewFlags, uint8 ClientRoll, uint32 View, UPrimitiveComponent* ClientMovementBase, FName ClientBaseBoneName, uint8 ClientMovementMode);
	virtual bool ServerMoveDual_Validate(float TimeStamp0, FVector_NetQuantize10 InAccel0, FVector_NetQuantize10 InReceivedImp0, uint8 PendingFlags, uint32 View0, float TimeStamp, FVector_NetQuantize10 InAccel, FVector_NetQuantize10 ReceivedImp, FVector_NetQuantize100 ClientLoc, uint8 NewFlags, uint8 ClientRoll, uint32 View, UPrimitiveComponent* ClientMovementBase, FName ClientBaseBoneName, uint8 ClientMovementMode);

	/* Resending an (important) old move. Process it if not already processed. */
	UFUNCTION(unreliable, server, WithValidation)
	virtual void ServerMoveOld(float OldTimeStamp, FVector_NetQuantize10 OldAccel, FVector_NetQuantize10 OldReceivedImpulse, uint8 OldMoveFlags);
	virtual void ServerMoveOld_Implementation(float OldTimeStamp, FVector_NetQuantize10 OldAccel, FVector_NetQuantize10 OldReceivedImpulse, uint8 OldMoveFlags);
	virtual bool ServerMoveOld_Validate(float OldTimeStamp, FVector_NetQuantize10 OldAccel, FVector_NetQuantize10 OldReceivedImpulse, uint8 OldMoveFlags);
...
public:
	...
	virtual void SetMoveFor(ACharacter* C, float DeltaTime, FVector const& NewAccel, FVector const& NewReceivedImp, class FNetworkPredictionData_Client_Character & ClientData);
