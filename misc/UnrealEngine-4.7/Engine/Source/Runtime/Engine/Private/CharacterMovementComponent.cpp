	...
	: Super(ObjectInitializer)
{
	...
	ClearJumpTimerDelegate.BindUFunction(this, FName("ClearJump"));
}
...
void UCharacterMovementComponent::TickComponent(float DeltaTime, enum ELevelTick TickType, FActorComponentTickFunction *ThisTickFunction)
{
	...
	if (CharacterOwner->Role > ROLE_SimulatedProxy)
	{
		...
		if( CharacterOwner->IsLocallyControlled() || (!CharacterOwner->Controller && bRunPhysicsWithNoController) || (!CharacterOwner->Controller && CharacterOwner->IsPlayingRootMotion()) )
		{
			...
			{
				ReplicateMoveToServer(DeltaTime, Acceleration, ReceivedImpulse);
			}
		}
...
void UCharacterMovementComponent::PerformMovement(float DeltaSeconds)
{
	...
	if (CharacterOwner->Role == ROLE_Authority)
	{
		/*ReceivedImpulse = RoundAcceleration(ReceivedImpulse);*/

		/*if (LastReceivedImpulse != ReceivedImpulse)*/ PendingImpulseToApply += ReceivedImpulse;
		/*if (!FVector::Coincident(LastReceivedImpulse, ReceivedImpulse)) PendingImpulseToApply += ReceivedImpulse;*/

		/*LastReceivedImpulse = ReceivedImpulse;*/

		//if (CharacterOwner->IsLocallyControlled()) ReceivedImpulse = FVector::ZeroVector;
	}

	const bool bIsClient = (GetNetMode() == NM_Client && CharacterOwner->Role == ROLE_AutonomousProxy);
	if (bIsClient)
	{
		/*ReceivedImpulseClient = RoundAcceleration(ReceivedImpulseClient);*/

		/*if (LastReceivedImpulse != ReceivedImpulseClient)*/ PendingImpulseToApply += ReceivedImpulseClient;
		/*if (!FVector::Coincident(LastReceivedImpulse, ReceivedImpulseClient)) PendingImpulseToApply += ReceivedImpulseClient;*/
		/*LastReceivedImpulse = ReceivedImpulseClient;*/
	}

	ReceivedImpulse = FVector::ZeroVector;
...
bool UCharacterMovementComponent::ClientUpdatePositionAfterServerUpdate()
{
	...
	for (int32 i=0; i<ClientData->SavedMoves.Num(); i++)
	{
		...
		MoveAutonomous(CurrentMove->TimeStamp, CurrentMove->DeltaTime, CurrentMove->GetCompressedFlags(), CurrentMove->Acceleration, CurrentMove->ReceivedImpulse);
...
void UCharacterMovementComponent::ReplicateMoveToServer(float DeltaTime, const FVector& NewAcceleration, const FVector& NewReceivedImpulse)
{
	check(CharacterOwner != NULL);
	...
	NewMove->SetMoveFor(CharacterOwner, DeltaTime, NewAcceleration, NewReceivedImpulse, *ClientData);
	...
	/*PendingImpulseToApply += NewMove->ReceivedImpulse;*/
	ReceivedImpulseClient = NewMove->ReceivedImpulse;
	...
	)
{
	...
	if (OldMove)
	{
		ServerMoveOld(OldMove->TimeStamp, OldMove->Acceleration, OldMove->ReceivedImpulse, OldMove->GetCompressedFlags());
	}
...
void UCharacterMovementComponent::ServerMoveOld_Implementation
	...
	FVector_NetQuantize10 OldReceivedImpulse,
	uint8 OldMoveFlags
	)
{
	...
	MoveAutonomous(OldTimeStamp, FMath::Min(OldTimeStamp - ServerData->CurrentClientTimeStamp, MaxResponseTime), OldMoveFlags, OldAccel, OldReceivedImpulse);
...
void UCharacterMovementComponent::ServerMoveDual_Implementation(
	...
	FVector_NetQuantize10 InReceivedImp0,
	uint8 PendingFlags,
	uint32 View0,
	float TimeStamp,
	FVector_NetQuantize10 InAccel,
	FVector_NetQuantize10 ReceivedImp,
...
{
	ServerMove_Implementation(TimeStamp0, InAccel0, InReceivedImp0, FVector(1.f, 2.f, 3.f), PendingFlags, ClientRoll, View0, ClientMovementBase, ClientBaseBone, ClientMovementMode);
	ServerMove_Implementation(TimeStamp, InAccel, ReceivedImp, ClientLoc, NewFlags, ClientRoll, View, ClientMovementBase, ClientBaseBone, ClientMovementMode);
}
...
void UCharacterMovementComponent::ServerMove_Implementation(
	float TimeStamp,
	FVector_NetQuantize10 InAccel,
	FVector_NetQuantize10 InReceivedImpulse,
	FVector_NetQuantize100 ClientLoc,
	uint8 MoveFlags,
	uint8 ClientRoll,
	uint32 View,
	...
	uint8 ClientMovementMode)
{
	...
	const FVector HitImpulse = InReceivedImpulse;
	...
	if ((CharacterOwner->GetWorldSettings()->Pauser == NULL) && (DeltaTime > 0.f))
	{
		...
		MoveAutonomous(TimeStamp, DeltaTime, MoveFlags, Accel, HitImpulse);
	}
	...
	// HACK: forcibly acknowledge receipt of this successful servermove()
	/*if (!InReceivedImpulse.IsZero())
	{
		FNetworkPredictionData_Server_Character* ServerData = GetPredictionData_Server_Character();
		check(ServerData);
	...
	ServerMoveHandleClientError(TimeStamp, DeltaTime, Accel/*, HitImpulse*/, ClientLoc, ClientMovementBase, ClientBaseBoneName, ClientMovementMode);
}
...
bool UCharacterMovementComponent::ServerMove_Validate(float TimeStamp, FVector_NetQuantize10 InAccel, FVector_NetQuantize10 InReceivedImpulse, FVector_NetQuantize100 ClientLoc, uint8 MoveFlags, uint8 ClientRoll, uint32 View, UPrimitiveComponent* ClientMovementBase, FName ClientBaseBoneName, uint8 ClientMovementMode)
{
	return true;
}

bool UCharacterMovementComponent::ServerMoveDual_Validate(float TimeStamp0, FVector_NetQuantize10 InAccel0, FVector_NetQuantize10 InReceivedImp0, uint8 PendingFlags, uint32 View0, float TimeStamp, FVector_NetQuantize10 InAccel, FVector_NetQuantize10 ReceivedImp, FVector_NetQuantize100 ClientLoc, uint8 NewFlags, uint8 ClientRoll, uint32 View, UPrimitiveComponent* ClientMovementBase, FName ClientBaseBoneName, uint8 ClientMovementMode)
{
	return true;
}

bool UCharacterMovementComponent::ServerMoveOld_Validate(float OldTimeStamp, FVector_NetQuantize10 OldAccel, FVector_NetQuantize10 OldReceivedImpulse, uint8 OldMoveFlags)
{
	return true;
}
...
void UCharacterMovementComponent::MoveAutonomous
	...
	const FVector& NewReceivedImpulse
	)
{
	if (!HasValidData())
	{
	...
	if (CharacterOwner->Role == ROLE_Authority) ReceivedImpulse = NewReceivedImpulse;
	if (CharacterOwner->Role == ROLE_AutonomousProxy) ReceivedImpulseClient = NewReceivedImpulse;
...
void UCharacterMovementComponent::ClientAckGoodMove_Implementation(float TimeStamp)
{
	...
	// ISSUE: in case of packet loss, JumpInput can be dropped before reaching the server.
	// WORKAROUND: do not clear 'bPressedJump' till appropriate move is acknowledged.
	//if (
	//	(ClientData->SavedMoves[MoveIndex].Get())->bPressedJump /*&&
	//	!GetWorld()->GetTimerManager().IsTimerActive(ClearJumpTimerHandle)*/
	//	)
	//{
	//	ClearJump();
	...
	//if (!(ClientData->SavedMoves[MoveIndex].Get())->ReceivedImpulse.IsZero())
	//{
	//	/*GEngine->AddOnScreenDebugMessage(0, 1, FColor::Yellow, L"ReceivedImpulse");*/
	//	ReceivedImpulse = FVector::ZeroVector;
	//}

	ClientData->AckMove(MoveIndex);

	//if (!ClientData->LastAckedMove->ReceivedImpulse.IsZero() || MoveIndex == INDEX_NONE)
	//{
	//	/*GEngine->AddOnScreenDebugMessage(0, 1, FColor::Yellow, L"ReceivedImpulse");*/
	//	ReceivedImpulse = FVector::ZeroVector;
	//	bReceivedImpulsePendent = false;
	//}
}

void UCharacterMovementComponent::ClearJump()
{
	Cast<ACharacter>(CharacterOwner)->bPressedJump = false;
}
...
void FSavedMove_Character::SetMoveFor(ACharacter* Character, float InDeltaTime, FVector const& NewAccel, FVector const& NewReceivedImpulse, class FNetworkPredictionData_Client_Character & ClientData)
{
	DeltaTime = InDeltaTime;
	...
	// This is done after the AccelMag and AccelNormal are computed above, because those are only used client-side for combining move logic and need to remain accurate.
	Acceleration = Character->GetCharacterMovement()->RoundAcceleration(NewAccel);
	ReceivedImpulse = Character->GetCharacterMovement()->RoundAcceleration(NewReceivedImpulse);
	bPressedJump = Character->bPressedJump;
	JumpKeyHoldTime = Character->JumpKeyHoldTime;
	bWantsToCrouch = Character->GetCharacterMovement()->bWantsToCrouch;
	bForceMaxAccel = Character->GetCharacterMovement()->bForceMaxAccel;
...
bool FSavedMove_Character::IsImportantMove(const FSavedMovePtr& LastAckedMove) const
{
	...
	if (ReceivedImpulse != LastAckedMove->ReceivedImpulse)
	{
		return true;
	}
	return false;
...
bool FSavedMove_Character::CanCombineWith(const FSavedMovePtr& NewMove, ACharacter* Character, float MaxDelta) const
{
	...
	else
	{
		return (NewMove->DeltaTime + DeltaTime < MaxDelta)
			...
			&& FVector::Coincident(ReceivedImpulse, NewMove->ReceivedImpulse)
			&& StartBaseRotation.Equals(NewMove->StartBaseRotation) // only if base hasn't rotated
			&& (CustomTimeDilation == NewMove->CustomTimeDilation);
	}
}
